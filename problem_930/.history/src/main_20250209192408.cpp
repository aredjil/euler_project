...
get_expected_steps(std::vector<int> &bowls, std::vector<int> &balls, const int &n_steps)
{
    ...
    std::vector<int> counts(n_steps, 0);
    ...
    double avg = static_cast<double>(sum) / n_steps;
    return avg;
}
...
int main(int argc, char **argv)
{
    ...
    std::vector<int> balls(m, 0); // Vector to hold track the location of the balls
    std::vector<int> bowls(n, 0); // Vector to track the size of the bowls

    // double sum = 0.0;
    // for (int i = 2; i < N; i++)
    // {
    //     std::vector<int> bowls(i, 0); // Vector to track the size of the bowls
    //     for (int j = 2; j < M; ++j)
    //     {
    //         std::vector<int> balls(j, 0); // Vector to hold track the location of the balls
    //         sum += get_expected_steps(bowls, balls, n_steps);
    //     }
    // }
    // std::cout << "G(3, 3): " << sum << "\n";
    auto avg = get_expected_steps(bowls, balls, n_steps);
